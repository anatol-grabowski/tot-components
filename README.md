...
- use colors and common styles from `core.js` to allow later implementing themes or dark mode
- component's `README.md` should include
  - "Description" section - general description mentioning what native element the component is based on
  - "Props" section - a table listing all custom props, their TypeScript types, default values, and descriptions, if read-only or not, and if they are reflected as attributes
  - "Events" section - a table listing all custom events emitted by the component, their types, and descriptions
  - "Slots" section - a table listing all slots, their descriptions listing props that they will receive with their types
  - "Usage" section - shows a simple example of usage (configuring necessary props, listening to events, etc.)
  - "Implementation" section - describes the approach to implementation, component's behavior, and any important details about the component's internal workings
  - avoid paragraphs of text in descriptions, instead use short bullet points (nested if necessary)
...
- `src/components/[ComponentName]/README.md`
  - contains documentation for the component
  - explains how to use the component, its props, and any other relevant information
...
- `src/components/[ComponentName]/[ComponentName].demo.js`
  ...
  - import from `demoCommon.js`
  - register themselves in `demoCommon.js` for rendering in `demo.html`
