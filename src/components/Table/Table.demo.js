  ...
  render: (container, { logEvent }) => {
    ...
    row.innerHTML = `
      <div class="stack demo-group">
        <div class="demo-label">Sticky rows and columns with type-* template slots</div>
        <tot-table id="largeTable" style="--tot-table-max-height: 18rem;">
          <template slot="type-corner"><th class="header">{{content}}</th></template>
          <template slot="type-group"><th class="group">{{content}}</th></template>
          <template slot="type-month"><th class="header numeric">{{content}}</th></template>
          <template slot="type-row-header"><th class="row-header">{{content}}</th></template>
          <template slot="type-status"><td>{{content}}</td></template>
          <template slot="type-total"><th class="total numeric">{{content}}</th></template>
        </tot-table>
      </div>
      <div class="stack demo-group">
...
function createLargeTable() {
  ...
  for (let rowIndex = 0; rowIndex < 30; rowIndex++) {
    ...
    for (let colIndex = 0; colIndex < months.length; colIndex++) {
      ...
      const className = `numeric${value >= 120 ? ' positive' : ''}${value < 65 ? ' warning' : ''}`
      row.push(cell(`score-${rowIndex}-${colIndex}`, 'score', value, className))
    }
...
function repeatCell(id, count, type, content, className) {
  const result = []
  for (let i = 0; i < count; i++) {
    result.push(cell(id, type, content, className, count))
  }
  return result
}

function cell(id, type, content, className = '', colspan, rowspan) {
  return {
    id,
    type,
    class: className,
